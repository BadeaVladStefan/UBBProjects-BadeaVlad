    ...
    public partial class Form1 : Form
    {
        ...
        void FillData()
        {
            try
            {
                ...
                if (!dataSet.Tables[parentTableName].Columns.Contains(parentPrimaryKey))
                {
                    throw new Exception($"Primary key column '{parentPrimaryKey}' not found in parent table.");
                }
                if (!dataSet.Tables[childTableName].Columns.Contains(childForeignKey))
                {
                    throw new Exception($"Foreign key column '{childForeignKey}' not found in child table.");
                }
                ...
                DGVParent.DataSource = dataSet.Tables[parentTableName];
                DGVChild.DataSource = dataSet.Tables[parentTableName];
                DGVChild.DataMember = "ParentChildRelation";
